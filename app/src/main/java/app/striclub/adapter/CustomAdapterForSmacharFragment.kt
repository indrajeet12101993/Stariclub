...
import kotlinx.android.synthetic.main.custom_samachar_fragemnt_recycler.view.*
import app.striclub.R
import app.striclub.callback.InterfaceAllNewsFeedSelectListner
import app.striclub.pojo.listnews.Result
import app.striclub.utils.TimeUtils


class CustomAdapterForSmacharFragment(val userFeedList: MutableList<Result>, private val listner: InterfaceAllNewsFeedSelectListner) : RecyclerView.Adapter<CustomAdapterForSmacharFragment.ViewHolder>() {
    //this method is returning the view for each item in the list
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CustomAdapterForSmacharFragment.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.custom_samachar_fragemnt_recycler, parent, false)

        return ViewHolder(v)
    }

    //this method is binding the data on the list
    override fun onBindViewHolder(holder: CustomAdapterForSmacharFragment.ViewHolder, position: Int) {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var like: Int? = null
        fun bindItems(post: Result, listner: InterfaceAllNewsFeedSelectListner, adapterPosition: Int) {

            Glide.with(itemView.context).load(post.news_image).into(itemView.iv_lekh_view)

            itemView.tv_headline.text = post.title
            itemView.tv_content.text = Html.fromHtml(post.news_content)
            //itemView.tv_content.setText(post.news_content)
