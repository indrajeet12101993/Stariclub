...
import app.striclub.activity.HomeNewsMoreReadActivity
...
class UserMyPostFragment : BaseFragment(), InterFaceAllfeedVideoSelectListner, IFragmentManager, SwipeRefreshLayout.OnRefreshListener, InterfaceStateSelectListner, InterfaceCitySelectListner {
...
//
    ...
    private fun getAllUserFeed(argument: String?) {
        ...
        mCompositeDisposable?.add(ApiRequestClient.createREtrofitInstance()
                .getUserAllHomeFeed2(dataManager.getUserId())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
    ...
    private fun showRadioButtonDialog(post: Post) {
        ...
        dialog.setTitle("I deny this post..")
    ...
    fun sujhaoDialog() {
        ...
        popDialog.setPositiveButton(android.R.string.ok) { dialog, which ->
            ...
            if (sujao.isEmpty()) {
                dialogView.et_sujaho.setError("Suggest us!")
    ...
    fun ShowRatingDialog() {
        ...
        dialogView!!.btn_profile_edit_kare.setOnClickListener {
            ...
            if (name.isNullOrEmpty() || state.isNullOrEmpty() || district.isNullOrEmpty() || village.isNullOrEmpty()) {
                showSnackBar(" required input field !")
            } else {
                popDialog.dismiss()
    ...
    private fun initState() {
        mAndroidStateList = ArrayList<app.striclub.pojo.state.Result>()
        ...
        dialog_State.setTitle(" select state!")
    ...
    private fun initCity() {
        ...
        dialog_city.setTitle(" selelct district")
        ...
        if (state_id.isEmpty()) {
            ...
            alertDialog.setMessage("selelct state")
            alertDialog.setButton(AlertDialog.BUTTON_NEUTRAL, "OK",
                    DialogInterface.OnClickListener { dialog, which ->
                        dialog.dismiss()
                        initState()
    ...
    private fun handleResponseUpdateProfile(response: ServerResponseFromUplaodImage) {
        ...
        dialogBuilder.setMessage("thanks for suggestions !")
